...
class TheTyrannyOfTheRocketEquationTest extends AnyFunSpec with Matchers {
  ...
  val example: Seq[Long] = Seq(
    ...
    116978L, 135420L, 88165L, 97926L, 114772L, 143455L, 53613L, 60408L, 94299L, 98996L, 142167L, 78063L, 98974L, 65392L,
    140263L, 126726L, 141285L, 111074L, 95977L, 124871L, 136636L, 81935L
  )
  ...
  describe("sumOfFuelRequirements") {
    ...
    testData.foreach(t => {
      it(s"should return '${t._2}' for '${t._1}'") {
        TheTyrannyOfTheRocketEquation.sumOfFuelRequirements(t._1) shouldBe t._2
      }
    })

    it("should return result for example data") {
      TheTyrannyOfTheRocketEquation.sumOfFuelRequirements(example) shouldBe 3479429L
    }
  }

  describe("sumOfFuelRequirementsCorrection") {
    val testData: Seq[(Seq[Long], Long)] = Seq(
      (Seq(12L), 2L),
      (Seq(14L), 2L),
      (Seq(1969L), 966L),
    ...
    testData.foreach(t => {
      it(s"should return '${t._2}' for '${t._1}'") {
        TheTyrannyOfTheRocketEquation.sumOfFuelRequirementsCorrection(t._1) shouldBe t._2
      }
    })

    it("should return result for example data") {
      TheTyrannyOfTheRocketEquation.sumOfFuelRequirementsCorrection(example) shouldBe 5216273
    }
  }
}
